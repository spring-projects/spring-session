...
- I want to {docs-url}/spring-boot/appendix/application-properties/index.html#application-properties.data.spring.data.redis.host[customize the Redis configuration] using Spring Boot properties
...
There are a {spring-framework-ref-docs}/core/beans/context-introduction.html#context-functionality-events[few ways to listen to application events] in Spring, we are going to use the `@EventListener` annotation.
