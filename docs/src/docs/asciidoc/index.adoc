...
| {gh-samples-url}httpsession-redis-lettuce[HttpSession with Redis and Lettuce]
| Demonstrates how to use Spring Session to replace the `HttpSession` with a Redis store using the https://github.com/mp911de/lettuce[Lettuce] Redis client.
| link:guides/httpsession-lettuce.html[HttpSession Lettuce Guide]
...
* <<httpsession-redis-jc-lettuce,Java Based Configuration using Lettuce>>
...
[[httpsession-redis-jc-lettuce]]
==== Redis Java Based Configuration using Lettuce

This section describes how to use Redis with the Lettuce client to back `HttpSession` using Java based configuration.
...
NOTE: The <<samples, HttpSession with Redis and Lettuce>> provides a working sample on how to integrate Spring Session and `HttpSession` using Java configuration.
You can read the basic steps for integration below, but you are encouraged to follow along with the detailed HttpSession Guide when integrating with your own application.

include::guides/httpsession-lettuce.adoc[tags=config,leveloffset=+3]
