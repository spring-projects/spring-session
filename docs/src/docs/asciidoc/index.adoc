...
The implementation supports `SessionDestroyedEvent` and `SessionExpiredEvent` through `SessionMessageListener`.
...
Spring Session relies on the expired and delete http://redis.io/topics/notifications[keyspace notifications] from Redis to fire a <<api-redisoperationssessionrepository-sessiondestroyedevent,SessionDestroyedEvent>> and <<api-redisoperationssessionrepository-sessiondestroyedevent,SessionExpiredEvent>> respectively.
It is the `SessionDestroyedEvent` or `SessionExpiredEvent` that ensures resources associated with the Session are cleaned up.
...
==== SessionDestroyedEvent and SessionExpiredEvent

`RedisOperationsSessionRepository` supports firing a `SessionDestroyedEvent` whenever a `Session` is deleted or a `SessionExpiredEvent` when it expires.
...
Firing `SessionDestroyedEvent` or `SessionExpiredEvent` is made available through the `SessionMessageListener` which listens to http://redis.io/topics/notifications[Redis Keyspace events].
