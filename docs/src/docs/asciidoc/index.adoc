...
| {gh-samples-url}Cassandra[Apache Cassandra]
| Demonstrates how to use Spring Session with Cassandra.
| link:guides/cassandra.html[Cassandra Guide]
...
[[httpsession-cassandra]]
=== HttpSession with Apache Cassandra

This section describes how to use Apache Cassandra to back `HttpSession` using a Java based annotation configuration.

NOTE: The <<samples, HttpSession Cassandra Sample>> provides a working sample on how to integrate Spring Session and `HttpSession` using Java configuration.
You can read the basic steps for integration below, but you are encouraged to follow along with the detailed HttpSession Guide when integrating with your own application.

include::guides/cassandra.adoc[tags=config,leveloffset=+3]
